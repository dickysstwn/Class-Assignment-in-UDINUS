<?php
    ...
    if (isset($_POST['addProduct'])) {
        ...
        if (in_array($extension,$extension_format)==true) {
            if ($size < 1044070) {
                ...
                $add = mysqli_query($con, "INSERT INTO product VALUE (null, '$nm_product', '$description','$name','$price','$qty')");
                if ($add) {
    ...
    if (isset($_POST['editProduct'])) {
        ...
        if (in_array($extension,$extension_format)==true) {
            if ($size < 1044070) {
                move_uploaded_file($file_tmp, 'assets/img/'.$name);
                $edit = mysqli_query($con, "UPDATE product SET nm_product = '$nm_product', description = '$description', img_product = '$name', price = '$price', qty = '$qty' WHERE id_product = '$id_product'");
                if ($edit) {
