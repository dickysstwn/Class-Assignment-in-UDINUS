    ...
    if (isset($_POST['addcart'])) {
        ...
        if ($qty > $get_var['qty']) {
            ...
            if (mysqli_num_rows($get_varCart)>0) {
                ...
                $addcart = mysqli_query($con, "UPDATE cart set qty = '$qtyNow', price = '$priceNow' WHERE id_product = '$id_product' and stat = '0' and id_user = '$id_user'");   
                if ($addcart) {
                    $stockproduct = $get_var['qty'] - $qty;
                    $update = mysqli_query($con, "UPDATE product SET qty = '$stockproduct' WHERE id_product = '$id_product'");
                    echo '<script>alert("Berhasil menambah stock");window.location.href="home.php";</script>';
                }
                else{
                    echo '<script>alert("Gagal menambahkan Stock");window.location.href="home.php";</script>';
                }   
                
            }
            else{
                $addcart = mysqli_query($con, "INSERT INTO cart VALUE ('','$id_user','$id_product','$nm_product','$img_product','$qty',0,'$total')");
                if ($addcart) {
                    $stockproduct = $get_var['qty'] - $qty;
                    $update = mysqli_query($con, "UPDATE product SET qty = '$stockproduct' WHERE id_product = '$id_product'");
                    echo '<script>alert("Berhasil menambahkan ke keranjang");window.location.href="home.php";</script>';
                }
                else{
                    echo '<script>alert("Gagal menambahkan ke keranjang");window.location.href="home.php";</script>';
                }   
            }
...
<body>
    ...
    <div class="container-fluid jumbotron text-center mt-5">
        ...
        <h1 class="display-4">OOTD</h1>
        <p class="lead">"Outfit Of The Day"</p>
    </div>
                ...
                <div class="col-lg-3 my-4">
                    <div class="card" style="width: 100%;">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text">Rp. <?php echo $a['price'] ?></p>
